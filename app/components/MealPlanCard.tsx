  ...
  return (
    ...
    >
      <Card className="h-full bg-white/70 backdrop-blur-md shadow-lg hover:shadow-xl hover:bg-white/80 transition-all duration-300">
        <CardHeader className="bg-gradient-to-r from-green-500 to-white-500 text-white rounded-t-lg">
            ...
            <div className="space-y-4">
              {Object.entries(meals).map(([mealType, mealsList]) => (
                <div key={mealType} className="space-y-2">
                  ...
                  {(Array.isArray(mealsList) ? mealsList : [mealsList]).filter(Boolean).map((meal: any, index: number) => (
                    <div key={index} className="bg-gray-50 rounded-lg p-3 shadow-sm">
                      <div className="font-medium text-primary">{meal.name || 'Delicious Meal'}</div>
